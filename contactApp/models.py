...
def post_save_resume(sender,instance,**kwargs):
    print('之前的原始状态',instance.__original_status)
    print("save之后的状态status",instance.status)
    email=instance.email
    EMAIL_FROM='442516456@qq.com'
    if instance.__original_status==1 and instance.status==2:
        email_title='gg通知：恒达科技招聘初试结果'
        email_body='恭喜您通过本企业初试'
        send_status=send_mail(email_title, email_body,EMAIL_FROM,[email])
        ###--自动生成word表格
        template_path=os.getcwd()+"/media/contact/recruit.docx"
        template=DocxTemplate(template_path)   ##pip install docxtpl
        ...
        filesavename='%s/media/contact/recruit/%s_%d.docx' %(os.getcwd(),instance.name,instance.id)
        template.save(filesavename)
